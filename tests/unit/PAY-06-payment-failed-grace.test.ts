...
// Store lock for PAY-06. Does not claim Stripe E2E Closed.
// Break: markStripeSubscriptionGrace rewriting validTo to now+3 days.
...
test("PAY-06 store: payment_failed grace must not shorten a paid validTo", async () => {
  ...
  const before = (await store.ensureProductData()).subscriptions.find((item) => item.stripeSubscriptionId === "sub_pay06");
  assert.ok(before);
  const originalValidTo = before.validTo;
  assert.ok(Date.parse(originalValidTo) - Date.now() > 30 * 24 * 60 * 60 * 1000);

  const graced = await store.markStripeSubscriptionGrace("sub_pay06");
  assert.ok(graced);
  assert.equal(graced.validTo, originalValidTo);
