...
test("business email validation permits only the configured email punctuation", () => {
  ...
  for (const email of ["learner+tag@example.com", "learner:tag@example.com", "learner!tag@example.com", "learner name@example.com", "learner@example!.com"]) assert.equal(isBusinessEmail(email), false, email);
});
