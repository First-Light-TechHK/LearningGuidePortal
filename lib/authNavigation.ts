export function authNavigationHref(locale: string, mode: "sign-in" | "sign-up", currentUrl: string) {
  ...
  const authPage = /\/(?:portal\/)?(?:sign-in|sign-up|forgot-password|reset-password|check-email|password-reset-sent|bind-email(?:\/verify)?|verify-email)\/?$/.test(current.pathname);
