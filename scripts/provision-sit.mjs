...
if (process.argv.includes('--deploy')) {
  ...
  const sourceConfig = { ...source.SourceConfiguration, AutoDeploymentsEnabled: false, CodeRepository: { ...source.SourceConfiguration.CodeRepository, SourceCodeVersion: { Type: 'BRANCH', Value: 'codex/sit-deployment' }, CodeConfiguration: { ConfigurationSource: 'API', CodeConfigurationValues: { Runtime: 'NODEJS_22', BuildCommand: 'npm ci && npm run build', StartCommand: 'npm run start -- -p 8080', Port: '8080', RuntimeEnvironmentVariables: variables, RuntimeEnvironmentSecrets: secretArns } } } };
