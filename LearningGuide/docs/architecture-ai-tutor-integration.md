...
**日期**：2026-08-03  
**主仓**：GitHub [`Ashley-AIHR/AITutor`](https://github.com/Ashley-AIHR/AITutor) → 目录 [`LearningGuide/`](../)  
**相关源码**：

- 运行中的 Next.js / App Runner 应用：仓库根目录（`app/`、`services/`、`Dockerfile`）
- 产品文档与 LMS 迁移落点：本目录 `LearningGuide/`
- 遗留 Learning Guide MVP（参考/待退役）：本地 `learning-guide-admin` + `learning-guide-backend`（Java/Vue）
...
- **推荐路径**：以本仓库 Next.js（AITutor）为 **系统主应用（AWS App Runner）**，把 Learning Guide 已落地的用户/课程/订单/管理能力 **迁移进同一代码库**；产品文档与迁移工件放在 **`LearningGuide/`**；Java 后端 **冻结新功能**，仅作过渡或最终退役。
...
```text
Ashley-AIHR/AITutor/
  LearningGuide/          # 产品文档与迁移工件（本目录）
  app/                    # Next 运行时（tutor + 后续 lms route groups）
  services/               # AITutor stores / VFS
  Dockerfile              # App Runner
```
...
1. **Week 0**：冻结 Java 新功能；主仓固定为 `Ashley-AIHR/AITutor`，产品目录为 `LearningGuide/`。
2. **Phase 1**：App Runner 部署生产骨架（Postgres + S3 持久化，告别纯本地 `data/`）。
