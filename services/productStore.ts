...
export async function getLearningOverview(userId: string) {
  return editData((data) => {
    ...
    // D1 LEARN-02: preview-only records must not mint a My Learning card.
    const courseIds = [...new Set(records.map((record) => record.courseId))].filter((courseId) => liveEntitlement(data, userId, courseId));
