...
export function isKnowledgeAdminPath(pathname: string) {
  return /^\/(?:knowledge|course|knowledge-wiki|source-materials|llm-draft|publish)(?:\/|$)/.test(pathname)
    || /^\/api\/(?:navigation|courses|chat-testing|dialogue-testing|dialogue|source-materials|draft|llm-draft|knowledge-draft|extract-material|releases|wiki)(?:\/|$)/.test(pathname);
}
...
function isAdminAllowedPath(pathname: string) {
  ...
  if (pathname.startsWith("/api/course-media/") || isKnowledgeAdminPath(pathname)) return true;
