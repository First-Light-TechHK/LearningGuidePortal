...
export default async function PaymentSuccessPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<{ orderId?: string }> }) {
  ...
  return <main className="portal-page portal-page-narrow"><header className="portal-header"><Link className="portal-brand" href={`/${locale}/portal`}><span className="portal-brand-mark">LG</span><span>{getMessages(locale).brand}</span></Link><Link href={`/${locale}/account/my-learning`}>{copy.title}</Link></header><section className="portal-detail"><p className="portal-eyebrow">{title}</p><h1>{title}</h1><p className="portal-lead">{description}</p><Link className="portal-button portal-button-primary" href={`/${locale}/account/my-learning`}>{copy.continue}</Link><p className="portal-detail-meta">{overview.entitlements.filter((entitlement) => entitlement.state === "active").length} {copy.activeEntitlements}</p></section></main>;
}
