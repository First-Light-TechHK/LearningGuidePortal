...
export default async function SignInPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<{ returnTo?: string; oauthError?: string }> }) {
  ...
  return <main className="portal-page portal-auth-page"><AuthForm locale={locale} mode="sign-in" copy={messages.auth} providerError={providerError} googleEnabled={googleConfigured()} wechatEnabled={wechatConfigured()} returnTo={returnTo?.startsWith("/") && !returnTo.startsWith("//") ? returnTo : `/${locale}/account/my-learning`} /></main>;
}
