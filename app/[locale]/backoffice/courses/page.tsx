...
export default async function BackofficeCoursesPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <main className="portal-page portal-page-narrow"><header className="portal-header"><Link className="portal-brand" href={`/${locale}/portal`}><span className="portal-brand-mark">LG</span><span>{getMessages(locale).brand}</span></Link><nav className="portal-nav"><Link href={`/${locale}/backoffice/portal`}>{getMessages(locale).portalEditor.heading}</Link><Link href={`/${locale}/backoffice/orders`}>{getMessages(locale).backoffice.orders.title}</Link><Link href={`/${locale}/backoffice/payment`}>{getMessages(locale).backoffice.payment.title}</Link><Link href={`/${locale}/portal`}>{getMessages(locale).portal.eyebrow}</Link></nav></header><CourseManager locale={locale} copy={getMessages(locale).backoffice} /></main>;
}
