...
export default async function MyLearningPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <main className="portal-page portal-account-page">
      <PortalHeader locale={locale} active="my-learning" signedIn />
      <div className="account-utility"><SignOutButton label={copy.signOut} locale={locale} /></div>
      ...
      <section className="account-dashboard" aria-labelledby="my-learning-heading">
        <div className="account-dashboard-main">
          ...
          <section className="account-panel" aria-labelledby="continue-heading">
            ...
            {overview.courses.length ? <div className="account-learning-list">{overview.courses.map((item) => { const canStudy = Boolean(item.entitlement); const isComplete = item.completedAt !== null || item.progress === 100; const actionLabel = canStudy ? (isComplete ? copy.review : copy.continue) : copy.viewPlans; return <article className="account-learning-card" key={item.id}><CourseThumbnail slug={item.courseId} title={item.courseTitle} /><div className="account-learning-card-content"><div className="account-learning-card-title"><div><h3>{item.courseTitle}</h3>{item.courseDescription ? <p>{item.courseDescription}</p> : null}</div><span className={`account-status ${isComplete ? "complete" : canStudy ? "active" : "expired"}`}>{isComplete ? copy.completed : canStudy ? copy.inProgress : copy.expired}</span></div><p className="account-learning-meta">{item.lessonCount} {copy.lessons.toLowerCase()} · {copy.progress}: {item.progress}%{item.currentLessonTitle ? ` · ${copy.currentLesson}: ${item.currentLessonTitle}` : ""}</p><div className="progress-track"><span style={{ width: `${item.progress}%` }} /></div><Link className="portal-button portal-button-primary account-learning-action" href={canStudy ? `/${locale}/account/learn/${item.courseId}` : `/${locale}/pricing?courseId=${encodeURIComponent(item.courseId)}`}>{actionLabel}</Link></div></article>; })}</div> : <div className="account-empty-state"><p>{copy.noCourses}</p><Link className="portal-button portal-button-primary" href={`/${locale}/portal/courses`}>{messages.portal.viewCourses}</Link></div>}
          </section>
          <section className="account-panel account-activity-panel" aria-labelledby="notifications-heading"><div className="account-panel-heading"><div><h2 id="notifications-heading">{copy.notifications}</h2><p>{locale === "en-GB" ? "Recent updates from Learning Guide." : "Learning Guide 的最近更新。"}</p></div><Link className="portal-text-link" href={`/${locale}/account/my-learning/notifications`}>{messages.notificationsPage.viewAll}</Link></div>{overview.notifications.length ? <div className="notification-list">{overview.notifications.slice(0, 3).map((notification) => <div className="notification-row" key={notification.id}><strong>{notification.title}</strong><span>{notification.body}</span></div>)}</div> : <p className="portal-empty">{messages.notificationsPage.empty}</p>}</section>
        </div>
