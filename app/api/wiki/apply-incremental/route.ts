...
export async function POST(request: Request) {
  try {
    const { courseId, knowledgeId, incrementalMarkdown } = (await request.json()) as {
      courseId?: string;
      knowledgeId?: string;
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Apply incremental wiki update failed" }, { status: 400 });
  }
}
