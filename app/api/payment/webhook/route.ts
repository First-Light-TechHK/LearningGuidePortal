...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (process.env.STRIPE_SANDBOX === "1" && event.livemode) return NextResponse.json({ ok: false, error: "Live events are not allowed in this sandbox." }, { status: 400 });
    if (["checkout.session.completed", "checkout.session.async_payment_succeeded"].includes(event.type)) {
      ...
      if (session.payment_status === "unpaid") return NextResponse.json({ ok: true, pending: true });
      ...
      if (subscriptionId && !await stripeEventProcessed(event.id)) {
        if (event.type === "invoice.paid") {
          ...
          if (remote.status === "trialing" && !invoice.amount_paid) return NextResponse.json({ ok: true, ignored: true });
