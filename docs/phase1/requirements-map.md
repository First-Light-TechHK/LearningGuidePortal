...
Password reset flow: `components/portal/AuthForm.tsx` carries email/returnTo to forgot-password; `PasswordResetRequestForm.tsx` sends the request and opens `/portal/password-reset-sent` with a 60-second resend cooldown and change-email/sign-in actions. `PasswordResetConfirmForm.tsx` shows a dedicated success state, removes the used token from the address bar and preserves returnTo when signing in. DEV mail delivery is implemented in `services/passwordResetService.ts` and `services/emailService.ts`, with bilingual UI and email content. Service/API regression: `tests/integration/password-reset.spec.ts`; bilingual browser flow: `tests/e2e/password-reset.spec.ts`. Live mailbox delivery and Figma visual acceptance remain target-environment release checks.

Remember password: `components/portal/AuthForm.tsx` saves the email and password in browser localStorage after successful checked sign-in, restores both on the next sign-in visit (including after logout), and clears both when unchecked. An explicit initial email only restores a saved password when it belongs to the same email address. Storage failures do not block authentication. Existing server session-cookie persistence is retained; no API or database migration is required. Target-environment smoke and Figma visual acceptance remain pending.
...
Email registration is implemented in `services/productStore.ts`, `services/emailService.ts`, `app/api/auth/register`, `app/api/auth/verify-email`, and `app/api/auth/resend-verification`. Email/password accounts remain pending until a one-time 24-hour verification token is accepted. Registration does not create a session before activation. Resend is explicit only—opening or copying a check-email URL does not send mail—and is persistently limited to once per 60 seconds and ten emails per rolling 24 hours; the page uses the server-provided remaining cooldown. The Portal check-email and verify-email pages cover resend, loading, success, invalid, expired, and already-used link states.

Google registration and sign-in are implemented in `services/oauthService.ts`, `app/api/auth/google`, and `app/api/auth/google/callback`. The server-side Authorization Code flow uses signed state, OIDC nonce, PKCE and verified ID Token claims. A first-created Google account uses the verified Google email prefix as Name; this Google-only exception permits email-prefix characters otherwise disallowed for manually entered Names, and later login never overwrites the saved Name. Accounts are keyed by `(provider, providerSubject)` and are never merged automatically by email.
