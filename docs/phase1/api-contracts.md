...
Email registration uses `POST /api/auth/sign-up`, activation uses `POST /api/auth/verify-email`, and resend uses `POST /api/auth/resend-verification`. Only an explicit resend action can request a link; loading the check-email page never sends mail. A pending account can receive at most one verification email every 60 seconds and ten within a rolling 24-hour period. A successful response includes `data.retryAfter`; the 60-second cooldown returns `429 VERIFICATION_RATE_LIMITED` with its server-calculated `data.retryAfter`, and the daily cap returns `429 VERIFICATION_DAILY_LIMIT`. Issuing a new link invalidates the previous one.
