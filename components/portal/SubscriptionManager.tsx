...
type Order = { id: string; status: "paid" | "pending" | "failed" | "canceled" | "refunded"; amountMinor: number; currency: string; paymentMode: "demo" | "stripe"; kind?: "purchase" | "trial_activation" | "upgrade"; createdAt: string; stripeInvoiceId?: string | null; plan?: { id?: string; name: string } | null };
type Copy = {
...
export function SubscriptionManager({ initialSubscriptions, initialOrders, locale, copy }: { initialSubscriptions: Subscription[]; initialOrders: Order[]; locale: "en-GB" | "zh-CN"; copy: Copy }) {
  ...
  const latestOrderFor = useMemo(() => (subscription: Subscription) => initialOrders.find((order) => order.status === "paid" && order.kind !== "trial_activation" && order.plan?.id === subscription.plan?.id), [initialOrders]);
  ...
  async function confirmCancel() {
    ...
    const updated = await updateSubscription(cancelTarget, "cancel", reason, reasonText);
  ...
  return <section className="subscription-section">
    ...
    {subscriptions.length ? <div className="subscription-table-wrap"><table className="subscription-table"><thead><tr><th>{copy.plan}</th><th>{copy.amount}</th><th>{copy.validPeriod}</th><th>{copy.receipt}</th><th aria-label="Actions" /></tr></thead><tbody>{subscriptions.map((subscription) => { const order = latestOrderFor(subscription); const receiptAvailable = Boolean(order?.stripeInvoiceId) || order?.paymentMode === "demo"; const canCancel = subscription.state === "active" && (subscription.source === "purchase" || subscription.source === "trial"); const canResume = subscription.state === "cancel_at_period_end" || (subscription.state === "trial_canceled" && new Date(subscription.validTo) > new Date()); return <tr key={subscription.id}><td><strong>{subscription.plan?.name || (subscription.source === "trial" ? copy.trial : copy.active)}</strong><span className={`subscription-state subscription-state-${subscription.state}`}>{stateLabel(subscription)}</span></td><td>{order ? `${(order.amountMinor / 100).toFixed(2)} ${order.currency.toUpperCase()}` : subscription.source === "trial" ? "USD 0.00" : "—"}</td><td>{new Date(subscription.validFrom).toLocaleDateString(locale)} – {new Date(subscription.validTo).toLocaleDateString(locale)}</td><td>{receiptAvailable && order ? <a className="table-link" href={`/api/my-learning/orders/${encodeURIComponent(order.id)}/receipt`} target="_blank" rel="noreferrer">{copy.receipt}</a> : <span className="muted-cell">{copy.noReceipt}</span>}</td><td><div className="table-actions">{canCancel ? <button className="portal-button portal-button-secondary" disabled={busyId === subscription.id} onClick={() => setCancelTarget(subscription)} type="button">{copy.cancel}</button> : null}{canResume ? <button className="portal-button portal-button-secondary" disabled={busyId === subscription.id} onClick={() => void updateSubscription(subscription, "resume")} type="button">{copy.resume}</button> : null}{subscription.state === "grace" ? <><button className="portal-button portal-button-secondary" onClick={() => void manage(subscription.id)} type="button">{copy.updatePaymentMethod}</button><button className="portal-button portal-button-primary" onClick={() => void manage(subscription.id, "pay")} type="button">{copy.payNow}</button></> : null}{subscription.source === "purchase" && subscription.state === "active" && subscription.scope === "category" ? <button className="portal-button portal-button-secondary" onClick={() => window.location.assign(`/${locale}/pricing?upgradeFrom=${encodeURIComponent(subscription.id)}`)} type="button">{copy.upgrade}</button> : null}</div></td></tr>; })}</tbody></table></div> : <p className="portal-empty">{copy.expired}</p>}
