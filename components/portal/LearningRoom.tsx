...
export function LearningRoom({ locale, courseId, lesson, initialCompleted, copy, fallbackImageUrl = null }: { locale: "en-GB" | "zh-CN"; courseId: string; lesson: { id: string; title: string; body: string; durationMinutes: number; contents?: LessonContent[] }; initialCompleted: boolean; copy: { study: string; minutes: string; you: string; completeLesson: string; completed: string; tutor: string; lecture: string; socratic: string; send: string; askTutor: string }; fallbackImageUrl?: string | null }) {
  ...
  return <div className="learning-room"><article className="lesson-content"><p className="portal-eyebrow">{copy.study}</p><h1>{lesson.title}</h1><p className="lesson-duration">{lesson.durationMinutes} {copy.minutes}</p>{lesson.contents !== undefined ? <LessonContentPlayer key={lesson.id} contents={lesson.contents} locale={locale} fallbackImageUrl={fallbackImageUrl}/> : <div className="lesson-body">{lesson.body.split("\n\n").map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div>}<button className="portal-button portal-button-primary" onClick={complete} disabled={completed}>{completed ? copy.completed : copy.completeLesson}</button></article><TutorChat locale={locale} courseId={courseId} lessonId={lesson.id} copy={copy} /></div>;
}
