...
type Plan = { id: string; name: string; termMonths: 6 | 12; device: "pc" | "mobile"; amountMinor: number; currency: string; aiPoints?: number; scope?: string; scopeId?: string | null; category?: string | null };
...
export function PricingPlans({ locale, plans, categories, selectedPlanId, copy }: { locale: Locale; plans: Plan[]; categories: PortalCategory[]; selectedPlanId?: string; copy: Copy }) {
  ...
  return <div className="pricing-comparison">{(["everything", "category"] as const).map((scope) => {
    ...
    return <article className="pricing-design-card" key={scope}>
      ...
      {plan ? <PurchasePanel key={plan.id} locale={locale} courseId="*" plans={[plan]} allowTrial={false} compact copy={{ buy: copy.subscribe, choosePlan: copy.term, startTrial: "" }} /> : <p role="status">{copy.unavailable}</p>}
    </article>;
  })}</div>;
}
