...
export function SettingsForm({ countries, initialNickname, initialEmail, initialLocale, initialCountry, initialAgeRange, initialEducation, initialAreasOfInterest, hasAvatar, copy }: { countries: string[]; initialNickname: string; initialEmail: string; initialLocale: "en-GB" | "zh-CN"; initialCountry?: string | null; initialAgeRange?: string | null; initialEducation?: string | null; initialAreasOfInterest?: string[]; hasAvatar: boolean; copy: Copy }) {
  ...
  async function uploadAvatar(file: File | undefined) {
    if (!file) return;
    setAvatarError("");
    if (!["image/jpeg", "image/png"].includes(file.type)) { setAvatarError("Use a JPG or PNG image."); return; }
  ...
  return <form className="portal-form settings-form" onSubmit={submit}>
    <div className="avatar-settings"><div className="avatar-preview">{avatar ? <img src={`/api/my-learning/avatar?v=${avatarVersion}`} alt={copy.avatar} /> : <span aria-hidden="true">{initial}</span>}</div><div><strong>{copy.avatar}</strong><p className="settings-hint">JPG / PNG · 5 MB · 1:1</p><div className="backoffice-row-actions"><label className="portal-button portal-button-secondary avatar-upload">{copy.uploadAvatar}<input type="file" accept="image/jpeg,image/png" onChange={(event) => void uploadAvatar(event.target.files?.[0])} /></label>{avatar ? <button className="portal-button portal-button-secondary" onClick={() => void removeAvatar()} type="button">{copy.removeAvatar}</button> : null}</div></div></div>
    ...
    <div className="settings-grid"><label>{copy.country}<select aria-label={copy.country} value={country} onChange={(event) => setCountry(event.target.value)} required><option value="">{copy.countryPlaceholder}</option>{countries.map((item) => <option key={item}>{item}</option>)}</select></label><label>{copy.ageRange}<select aria-label={copy.ageRange} value={ageRange} onChange={(event) => setAgeRange(event.target.value)}><option value="">{copy.agePlaceholder}</option>{ageRanges.map((item) => <option key={item}>{item}</option>)}</select></label><label>{copy.education}<select aria-label={copy.education} value={education} onChange={(event) => setEducation(event.target.value)}><option value="">{copy.educationPlaceholder}</option>{educationLevels.map((item) => <option key={item}>{item}</option>)}</select></label><label>{copy.language}<select value={locale} onChange={(event) => setLocale(event.target.value as "en-GB" | "zh-CN")}><option value="en-GB">{copy.english}</option><option value="zh-CN">{copy.chinese}</option></select></label></div>
    <label>{copy.areasOfInterest}<select aria-label={copy.areasOfInterest} multiple size={5} value={areasOfInterest} onChange={(event) => setAreasOfInterest(Array.from(event.target.selectedOptions).map((option) => option.value).slice(0, 5))}>{interestOptions.map((item) => <option key={item} disabled={areasOfInterest.length >= 5 && !areasOfInterest.includes(item)}>{item}</option>)}</select><span className="settings-hint">{copy.areasHint} ({areasOfInterest.length}/5)</span></label>
